...
/*   Updated: 2025/11/30 12:18:59 by myivanov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	redirect_input(t_cmdtree *cmdtree, t_list *token_node, int *hdocs)
{
	...
	if (token->control == 7)
	{
		temp = open(token_next->str, O_RDONLY);
		if (temp != -1)
		{
	...
	{
		cmdtree->fd_input = token_next->fd_heredoc;
		cmdtree->is_local_fd_input = 1;
		remove_heredoc(hdocs, token_next->fd_heredoc);
	}
	return (temp);
}
