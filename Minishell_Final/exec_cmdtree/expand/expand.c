...
/*   Updated: 2025/11/29 18:25:35 by causilva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	expand_wild(t_list **token_ptr, t_vars *vars);
static void	replace(t_list **token_ptr, t_list *expanded_lst, int is_wild);
static int	remove_quotes_token(t_token *token);
...
int	expand(t_list **tokens_ptr, t_vars *vars)
{
	...
	if (expand_env(tokens_ptr, vars) == -1)
		return (-1);
	if (expand_wild(tokens_ptr, vars) == -1)
		return (-1);
	while (*tokens_ptr)
	{
...
static int	remove_quotes_token(t_token *token)
{
	...
	if (!token || !token->str || !token->quoted)
		return (-1);
	i = 0;
